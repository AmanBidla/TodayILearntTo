...
bool load(string s);
bool lookup(string s,int min,int max);
void scramble(void);
...
int main(int argc, string argv[])
{
    ...
    while (true)
    {
        ...
        // after making word uppercase
        else
        {
            for(int i=0;i<strlen(s);i++){
                s[i]=toupper(s[i]);
            }
            if (find(s) && lookup(s,0,dictionary.size))
                score += strlen(s);
        }
    }
...
bool lookup(string s,int min, int max)
{
    /* if max < min return no more word to search */
    if(max<min) return false;
    
    /* find the middle of dictionary array */
    int mid=min+((max-min)/2);

    /* compare the dictionary word with search word */
    int compare=strcmp(dictionary.words[mid].letters,s);

    /* if dictionary word is found before search word then 
        search for all words to the right of the dictionary word */
    if(compare>0){
        max=mid-1;
        return lookup(s,min,max);
    }/* if dictionary word is found after search word then 
        search for all words to the left of the dictionary word */
    
    else if(compare<0){
        min=mid+1;
        return lookup(s,min,max);
     /* if dictionary word is found then 
          if its already flagged as found dont score
          else score the word and flag word as found */
    }else if(compare==0){
    ...
    /* this means word is never found is dictionary return false */
    return false;
